...
class B3bRun(G4Run):
    ...
    def __init__(self):
        ...
        self.fPrintModulo = 10000
        self.fGoodEvents = 0
        self.fSumDose = 0
        self.fStatDose = G4StatAnalysis()

    def RecordEvent(self, event):
        if self.fCollID_cryst < 0:
            self.fCollID_cryst = G4SDManager.GetSDMpointer().GetCollectionID("crystal/edep")
            # print(" fCollID_cryst:", self.fCollID_cryst)

        if self.fCollID_patient < 0:
            self.fCollID_patient = G4SDManager.GetSDMpointer().GetCollectionID("patient/dose")
            # print(" fCollID_patient:", fCollID_patient)

        evtNb = event.GetEventID()
        if evtNb % self.fPrintModulo == 0:
            print("---> end of event:", evtNb)
    ...
    def Merge(self, aRun):
        self.fGoodEvents += aRun.fGoodEvents
        self.fSumDose += aRun.fSumDose
        self.fStatDose += aRun.fStatDose
        super().Merge(aRun)


class B3bRunAction(G4UserRunAction):
    ...
    def EndOfRunAction(self, run):
        ...
        nbGoodEvents = run.fGoodEvents
        sumDose = run.fSumDose
        statDose = run.fStatDose
        ...
        statDose /= gray
        print(", Nb of 'good' e+ annihilations:", nbGoodEvents)
        print(" Total dose in patient:", G4BestUnit(sumDose, "Dose"))
        print(" Total dose in patient:", statDose, "Gy")
        print("------------------------------------------------------------")
...
class B3aActionInitialization(G4VUserActionInitialization):
    ...
    def BuildForMaster(self):
        self.SetUserAction(B3bRunAction())
